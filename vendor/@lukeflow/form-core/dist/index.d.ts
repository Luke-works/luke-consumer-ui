...
interface FormSettings {
    ...
    /** Allow the submitter to attach files to the submission. */
    attachments?: boolean;
    /** Render the completed submission to a PDF and attach it to the process instance
     *  (visible in the Form Inbox + Core UI Tasklist). Generated server-side on submit. */
...
/**
 * The id of the first author-placed SUBMIT button, or `null` if the form has none.
 *
 * <p>Lets a renderer avoid drawing a second submit button under an author who already placed their
 * own ("Submit Order", "Send enquiry"): two buttons that do the same thing read as a mistake, and the
 * author's wording is the one the filler should see.
 *
 * <p>Walks only what is REACHABLE from `root` — deliberately not {@link orderedIds}, which also
 * yields orphans. An orphaned button entity renders nothing, so counting it would suppress the
 * renderer's own button in favour of one that isn't on the page.
 */
declare function submitButtonId(schema: FormSchema): string | null;
...
/** Whether the submitter may attach files to the submission. */
declare function readAttachmentsEnabled(rawSchema: string | null | undefined): boolean;
/** Whether a completed submission is rendered to a PDF and attached to the process instance. */
declare function readSaveSubmissionAsPdf(rawSchema: string | null | undefined): boolean;
...
/** One address suggestion from a provider minion: a human-readable `label` + the structured value
 *  to fill on select. `id` is the provider's opaque result id (optional, for keys/dedupe). */
...
/**
 * Shape a provider minion's response into a list of {@link AddressSuggestion}. Reads the array via the
 * data source's `resultPath`, else accepts the response itself, a `{ results: [...] }`, or a
 * `{ suggestions: [...] }` envelope. Each item is normalized tolerantly (see {@link normalizeSuggestion}).
 */
declare function toAddressSuggestions(result: unknown, ds?: DataSource | null): AddressSuggestion[];
...
/**
 * Derive a form's DATA CONTRACT from its schema — the design-time answer to
 * "what data does this form produce, and how does each field accept it?".
 *
 * For every keyed field it reports: the submission `key`, the value `type`
 * (string/number/boolean/date/array/object, refined to `string[]` / `row[]` /
 * `file[]` for display), a human "accepts" description, and the field's
 * constraints (required, length/range/pattern, selection/row/file counts,
 * option count) — read exactly as the built-in validators read them. It also
 * emits an EXAMPLE submission payload, so a viewer can show the literal JSON
 * shape an integration receives.
 *
 * This mirrors the runtime submission shape produced by {@link import("./engine/createFormEngine").createFormEngine}'s
 * `collect()`: the payload is a FLAT map keyed by field `key`; layout containers
 * (`panel`/`columns`/…) flatten and contribute no key; grids (`dataGrid`/`editGrid`)
 * are the only nesting — an array of per-row key→value maps. `persistent: false`
 * fields are excluded from the payload (and from the example) but still listed
 * (flagged `excluded`), and conditionally-shown fields are flagged `conditional`.
 *
 * Pure, tolerant (never throws on a malformed draft), and DOM/React-free — the
 * same disciplines as {@link toPrintableHtml}, whose tree-walk this follows.
 *
 * @packageDocumentation
 */

/** The value category of a field, as surfaced in the contract. */
type DataValueType = FieldType["valueType"];
/**
 * One field in the derived data contract. `children` is populated for grids (the
 * per-row template fields) and for `addressBlock` (its fixed sub-keys); it is
 * `undefined` for scalar fields.
 */
interface DataContractField {
    /** The submission/scope key — the field's name in the payload. */
    key: string;
    /** The entity id (so a viewer can jump to / select the field). */
    entityId: string;
    /** The entity `type` discriminator (e.g. `email`, `dataGrid`). */
    fieldType: string;
    /** Human-readable label (falls back to the key when unlabelled). */
    label: string;
    /** The raw value category from the field-type registry. */
    valueType: DataValueType;
    /** Display type, refined for arrays/grids: `string`, `number`, `string[]`, `row[]`, `file[]`, `object`, … */
    typeLabel: string;
    /** How the field accepts input, in words (e.g. "Email address", "Multiple choice", "Repeating rows"). */
    accepts: string;
    /** Constraint chips read from the field's attributes (e.g. `required`, `≤ 80 chars`, `≥ 1 selected`). */
    constraints: string[];
    /** `true` when visibility is conditional (may be absent from a given submission). */
    conditional: boolean;
    /** `true` when `persistent: false` — the field is excluded from the submission payload. */
    excluded: boolean;
    /** Per-row template fields (grids) or fixed sub-keys (`addressBlock`); else undefined. */
    children?: DataContractField[];
    /** A representative example value, used to assemble {@link DataContract.example}. */
    example: unknown;
}
/** The whole derived contract: the ordered field list plus an example payload. */
interface DataContract {
    /** Every keyed field, in traversal order; containers flattened, grids nested. */
    fields: DataContractField[];
    /** An example submission payload (excludes `persistent: false` fields; grids/addressBlock nest). */
    example: FormData;
}
/**
 * Derive the data contract for a form schema. Optionally pass a custom
 * {@link FieldTypeRegistry}; defaults to the standard field set.
 */
...
/** A fixed metadata column the host prepends (e.g. who the row is being sent to). */
interface TemplateMetaColumn {
    key: string;
    label: string;
    type: string;
    /** `true` when the column must always be filled. */
    required: boolean;
    /**
     * Columns sharing a group id are "at least one of these" — none is individually required, but a
     * row with all of them blank is incomplete. Rendered as a note; there is no import to enforce it.
     */
    requiredGroup?: string;
    example: unknown;
}
/** How to decide which fields belong in the template. */
interface TemplateOptions {
    /**
     * Keep only these submission keys. Used for OUTBOUND forms, where the template exists for the
     * PREPARER to fill: a column for a field only the recipient can answer is noise at best, and at
     * worst reads as an instruction to answer on their behalf.
     *
     * Omit to include every fillable field (the inbound/default behaviour).
     */
    includeKeys?: readonly string[];
    /** Fixed columns prepended before the form's own fields. */
    metaColumns?: readonly TemplateMetaColumn[];
}
...
interface FormTemplate {
    /** Fixed metadata columns, before the form's own fields. */
    metaColumns: TemplateMetaColumn[];
    /** One column per included fillable field, in form order. */
 ...
 */
declare function buildFormTemplate(schema: FormSchema, registry?: FieldTypeRegistry, options?: TemplateOptions): FormTemplate;
