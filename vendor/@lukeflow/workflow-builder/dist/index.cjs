...
function TaskNode({ data, selected }) {
  const d = data;
  const m = capMeta(d.capability);
  return /* @__PURE__ */ (0, import_jsx_runtime2.jsx)(Card, { selected, severity: d.severity, accent: "var(--wf-accent)", children: titled("var(--wf-accent)", "\u{1F464}", d.label, `${d.capability ?? ""} task`.trim()) });
}
function TriggerNode({ data, selected }) {
  const d = data;
  return /* @__PURE__ */ (0, import_jsx_runtime2.jsx)(Card, { selected, severity: d.severity, accent: "#16a34a", target: false, style: { borderRadius: 999, background: "rgba(22,163,74,0.12)" }, children: titled("#16a34a", "\u26A1", d.label, "trigger") });
}
function WaitNode({ data, selected }) {
  const d = data;
  return /* @__PURE__ */ (0, import_jsx_runtime2.jsx)(Card, { selected, severity: d.severity, accent: "var(--wf-muted)", children: titled("var(--wf-muted)", "\u23F1", d.label, "wait") });
}
function GatewayNode({ data, selected, glyph, kind }) {
  const d = data;
  return /* @__PURE__ */ (0, import_jsx_runtime2.jsx)(Card, { selected, severity: d.severity, accent: "#d97706", style: { background: "rgba(217,119,6,0.14)" }, children: titled("#d97706", glyph, d.label, kind) });
}
