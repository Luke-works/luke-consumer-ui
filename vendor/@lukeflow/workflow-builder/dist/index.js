// src/graph.ts
import { END_NODE as END_NODE2, isTerminal as isTerminal2, nodeRefs as nodeRefs2, startNodeId as startNodeId2 } from "@lukeflow/workflow-core";

// src/layout.ts
import { isTerminal, nodeRefs, startNodeId } from "@lukeflow/workflow-core";

// src/ids.ts
import { END_NODE } from "@lukeflow/workflow-core";
...
function computeAutoLayout(doc) {
  ...
  for (const n of real) {
    for (const ref of nodeRefs(n)) {
      const t = isTerminal(ref.target) ? END_ID : ref.target;
...
function docToFlow(doc) {
  ...
  if (start) {
    ...
    while (queue.length > 0) {
      ...
      const n = byId.get(id);
      if (n) {
        for (const ref of nodeRefs2(n)) if (!depth.has(ref.target)) queue.push([ref.target, d + 1]);
      }
    }
  }
  ...
  const push = (source, target2, role, label2) => {
    const t = isTerminal2(target2) ? END_ID : target2;
  ...
  for (const n of doc.nodes ?? []) {
    ...
    switch (n.kind) {
      ...
      case "task":
        ...
        if (n.onError && !isTerminal2(n.onError.fallback)) push(n.id, n.onError.fallback, "fallback");
        break;
      case "wait":
        push(n.id, n.next, "next");
        break;
...
function flowToDoc(flow, base) {
  ...
  const mapTarget = (t) => t === END_ID ? END_NODE2 : t;
  const nodes = [];
  for (const fn of flow.nodes) {
    ...
    switch (n.kind) {
      ...
      case "wait": {
        ...
        n.next = next ? mapTarget(next.target) : END_NODE2;
      ...
      case "branch": {
        ...
        n.else = elseEdge ? mapTarget(elseEdge.target) : END_NODE2;
        break;
      }
      case "parallel": {
...
// src/operations.ts
import { END_NODE as END_NODE3, repairWorkflow } from "@lukeflow/workflow-core";
...
function addStep(doc, descriptor) {
  ...
  const node = descriptor.kind === "task" ? { id, kind: "task", capability, task: op, next: END_NODE3 } : { id, kind: "action", capability, action: op, next: END_NODE3 };
...
function addStructural(doc, kind) {
  ...
  switch (kind) {
    case "branch":
      node = { id, kind: "branch", conditions: [], else: END_NODE3 };
      break;
    case "parallel":
      node = { id, kind: "parallel", branches: [], join: END_NODE3 };
      break;
    case "wait":
      node = { id, kind: "wait", mode: "timer", duration: "PT1H", next: END_NODE3 };
      break;
  }
  return { ...doc, nodes: [...doc.nodes ?? [], node] };
}
...
// src/config.tsx
import { END_NODE as END_NODE4 } from "@lukeflow/workflow-core";
...
function nodeOptions(doc, excludeId) {
  ...
  opts.push({ value: END_NODE4, label: "\u25AA End" });
  return opts;
}
...
function TargetSelect({
  ...
  const v = value == null || value === "" ? END_NODE4 : value;
...
function InputField({ field, value, onChange }) {
  ...
  let node;
  if (type === "textarea") {
    node = /* @__PURE__ */ jsx("textarea", { ...common, rows: 3, value: String(value ?? ""), onChange: (e) => onChange(e.target.value) });
  } else if (type === "number") {
    node = /* @__PURE__ */ jsx("input", { ...common, type: "number", value: value == null ? "" : String(value), onChange: (e) => onChange(e.target.value === "" ? void 0 : Number(e.target.value)) });
    ...
    node = /* @__PURE__ */ jsx("input", { ...common, value: String(value ?? ""), onChange: (e) => onChange(e.target.value) });
  }
...
function NodeConfig({ doc, node, stepTypes = [], connections = [], onPatch, onDelete }) {
  ...
  return /* @__PURE__ */ jsxs("div", { children: [
    ...
    node.kind === "action" ? /* @__PURE__ */ jsxs(Fragment, { children: [
      ...
      /* @__PURE__ */ jsx(Section, { title: "Inputs", children: inputsEditor }),
...
function BranchConfig({
  ...
  const addCond = () => onPatch({ conditions: [...conditions, { expr: "", next: END_NODE4 }] });
...
function ParallelConfig({
  ...
  const addBranch = () => onPatch({ branches: [...branches, END_NODE4] });
...
function Card({
  ...
  return /* @__PURE__ */ jsxs2(
    "div",
    {
      ...
      children: [
        target2 ? /* @__PURE__ */ jsx2(Handle, { type: "target", position: Position.Left }) : null,
        children,
        source ? /* @__PURE__ */ jsx2(Handle, { type: "source", position: Position.Right }) : null
      ]
    }
  );
}
...
var EndNode = (p) => /* @__PURE__ */ jsx2(TerminalNode, { ...p, source: false, target: true, label: "End" });
var nodeTypes = {
  ...
  return /* @__PURE__ */ jsxs3(
    "div",
    {
      ...
      children: [
        ...
        onChange && (triggerSelected || selected) ? /* @__PURE__ */ jsx3(
          ConfigModal,
          {
    ...
    "div",
    {
      ...
      children: /* @__PURE__ */ jsxs3(
        "div",
        {
          ...
          children: [
            /* @__PURE__ */ jsxs3("div", { style: { display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, padding: "14px 18px", borderBottom: "1px solid var(--wf-border)", flex: "0 0 auto" }, children: [
              /* @__PURE__ */ jsxs3("div", { style: { minWidth: 0 }, children: [
                /* @__PURE__ */ jsx3("div", { style: { fontWeight: 700, fontSize: 15, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }, children: title }),
                subtitle ? /* @__PURE__ */ jsx3("div", { style: { fontSize: 11, color: "var(--wf-muted)", marginTop: 2 }, children: subtitle }) : null
              ] }),
              /* @__PURE__ */ jsx3(
                "button",
                {
            ...
            /* @__PURE__ */ jsx3("div", { style: { padding: 18, overflowY: "auto", fontSize: 13 }, children })
          ]
        }
      )
    }
